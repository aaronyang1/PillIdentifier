    ...
    for epoch_cnt in range(0, length_input // 5 * 4):
        ...
        np.random.shuffle(train_idxs):
            ...
            for batch_cnt in range(0, len(train_idxs)//batch_size):
                ...
                #same issue- not sure about input_data variable; needs to be defined     batchinput_imagesdata["images"][i]["id"] for i in i#change name of variable, it should not be input_images
                #conf_ids = [input_data["images"][random.randint(0, length_input)]["id"] for i in range(batch_size)] #change input_images
